...
- ❌ 套话开场/收尾：「在当今时代」「随着...的发展」「综上所述」「总而言之」「首先...其次...最后...」「不可否认」「毋庸置疑」「值得注意的是」「众所周知」「近年来」「由此可见」「值得我们深思」「给我们带来了」「提供了新的思路」「开启了...的新篇章」
- ❌ 模糊量词：「很多」「一些」「大量」「大多数」「少数」「若干」
- ❌ 重复前文的总结表格（每节已经讲过，最后再来个表复述 = AI 标志）
- ❌ 反复用「我们」「大家」「小伙伴」「老铁」等讨好型称呼
- ❌ 对称句式：「不仅...而且...」「既...又...」「一方面...另一方面...」（AI 喜欢工整对仗，真人很少这么写）
- ❌ 无意义铺垫：「在我看来」「个人认为」「我觉得」（真人直接说观点，不加这种 cushion）
...
- ✅ 关键的技术点可以用「错误写法 → 正确写法」代码对照（不是所有技术点都要，避免每节都用相同结构）
...
【图片占位符 - 通用规则】（所有类别必须遵守）
- ✅ \`[[IMG: ...]]\` 描述必须**自包含**：写完后 AI 绘图模型不需要看文章上下文就能画
- ✅ 用具象名词（节点 / 表格 / 箭头 / 人物 / 食物 / 场景），不用抽象词
- ✅ 长度建议：30-80 字
- ❌ 严禁抽象描述：「对比 A 和 B」「展示 X 流程」「表达 Y 概念」「示意图」（这些 AI 不知道画什么）
- ❌ 严禁在描述里出现具体文字（AI 绘图模型生成中文会乱码）
- ❌ 严禁让 AI 在图里写人名/品牌名/数字等具体字符

【结构反 AI 化 - 严格执行】
- ❌ 严禁「N 个 X」「5 个 Y」「3 招 Z」式清单化结构标题（AI 标志，真人作者很少这么写）
- ❌ 严禁每节都用相同的子结构（如「现象/原因/错误/正确」四件套）
- ❌ 严禁「综合最佳实践」「一次性总结」「完整模板」这种 AI 套路收尾
- ❌ 严禁用「首先...其次...最后...」「第一步...第二步...第三步...」式对称编号（除非真的是步骤教程）
- ❌ 严禁用「X 个教训」「Y 个误区」「Z 个技巧」这种偷懒结构
- ✅ 真人作者写得不对称、有详略：重点详写、次要略写，不要每个点都用相同长度展开
- ✅ 用具体场景/具体数字/具体时间填充内容（如「上个月」「3.2 万美元」「2 周」），不要用结构填充字数

【句式反 AI 化 - 严格执行】
- ❌ 严禁句子长度过于均匀（AI 标志：每句 15-25 字）。真人长短句交替：有的句子 5-10 字（短促有力），有的 50+ 字（一段话塞进一句）
- ❌ 严禁段落长度整齐划一（AI 标志：每段 3-5 句）。真人有的段落 1-2 句（一行结束），有的段落 8-10 句（一整屏展开）
- ❌ 严禁过度使用句号。真人多用问号（「为什么？」「真的？」「X 真的好用吗？」）、感叹号（「就这样！」「坑死人！」）、省略号（「然后...」「就这样没了」）
- ✅ 偶尔用「我靠」「说白了」「就这么简单」「别笑，真有人这么写」这类口语
- ✅ 偶尔用反问、设问、跳出叙述的口吻（你以为...？/ 说真的 / 我后来才意识到）
- ❌ 严禁 AI 高频废话句尾：「...给我们带来了...」「...提供了新的思路」「...值得我们深思」「...开启了...的新篇章」

【细节质量 - 必须做到】
- ✅ 案例具体到时间/地点/人物/数字：不要「有一次我朋友说...」，要「上个月在 V2EX 看到一个帖子，楼主说他用 X 公司 API 花了 3.2 万美元...」
- ✅ 引用具体来源：「XX 在他那篇《Y》里提到...」「根据 IDC 2024 报告...」「Stack Overflow 2024 调研显示...」
- ✅ 提到具体公司、产品、版本、年份：不要「一些浏览器」，要「Chrome 126 (2024.06)」「Next.js 15」
- ✅ 承认局限（不是 AI 的弱化句）：「这块我没完全搞懂」「可能不对，但我实测过...」「我朋友不同意这个观点」
- ✅ 留白和钩子：「这部分要细讲的话需要另开一篇」「如果你想深入了解 X，留言告诉我」
- ✅ 敢下判断：「X 就是被高估的」「Y 是 2024 最好的方案」「我推荐 Z，不推荐 W」

【可参考的结构模式】（仅作参考，不要当成模板照搬）

模式 A（技术/教程/工具类）：
- 标题：直接表达文章核心内容，不要清单化标题（不要「5 个 X」「N 个 Y」）
- 开头：直接进入第一个具体场景/案例（不要前言、不要"今天我们来聊聊"）
- 主体：自然展开，有详有略
  - 有的点详细讲（含代码/原理/方案），有的简略带过（一两段就够）
  - 不要每节都用相同的子结构（AI 标志）
  - 叙述要有节奏：长短句交替，重点段落长，过渡段落短
- 结尾：可以是具体行动建议、完整代码示例、或一句金句
  - 不要「综合最佳实践」「完整模板」这种 AI 套路收尾
...
【代码格式 - 严格执行】
- 所有注释一律放在代码块内、代码上方一行（// comment 在 \`\`\` 内、紧贴代码前一行）
- ❌ 严禁行尾注释：绝不能出现 \`code(); // 注释\` 这种格式
- 例：
  \`\`\`js
  // 输出：{ name: 'Hello' }
  obj.showThis();
...
【技术示意图 - 严格执行】（技术类必须严格遵守，保证生成图片稳定、准确、零误差）
- 技术类文章配图必须是**结构化示意图**，不是摄影/插画
- 图类型只能是：架构图、流程图、时序图、对比图、状态机图、树形图、表格图
- 视觉规范（强一致，确保稳定）：白底 / 扁平 / 线条清晰 / 配色 ≤3 色（蓝/灰/橙）/ 统一字号 / 无装饰 / 无文字 / 无人物 / 无光影
- 描述结构（必须按这个顺序，缺一项视为不合格）：
  1. 图类型（架构图 / 流程图 / 时序图 / 对比图 / 状态机图 / 表格图）
  2. 元素清单（每个节点/组件用具象名词：Master 节点、Shard 节点、API 网关、数据库表、服务 A、服务 B）
  3. 关系/连接（箭头方向：A → B，B ← C，双向）
  4. 布局（上下分层 / 左右并列 / 矩阵 / 树形 / 环形）
  5. 视觉规范（白底、扁平、配色）
- ❌ 严禁摄影/插画描述（俯拍、特写、暖光、冷光、扁平插画、电影感、写实、3D）— 这些会让 AI 绘图模型偏离示意图风格，生成不准确的图
- ❌ 严禁装饰性描述（干净背景、简约、现代、专业）— 与准确性无关，是废话
- ❌ 严禁模糊词（漂亮、好看、精致、有质感）

✅ 正例 1（架构图）：\`[[IMG: 架构图，左侧 1 个大矩形节点（标字母 M），3 条箭头指向右侧 3 个矩形节点（分别标 S1 S2 S3），每个矩形节点内有 3 个小表格图标，布局为左右并列，白底，扁平风格，蓝色和灰色配色，无文字]]\`

✅ 正例 2（对比图）：\`[[IMG: 对比图，左列 3 行图标分别是文件夹结构、函数符号、流程框，右列 3 行图标分别是文件树、组件符号、流程框，左右两列用虚线分隔，左列灰色配色右列蓝色配色，布局为左右并列，白底，扁平，无文字]]\`

✅ 正例 3（时序图）：\`[[IMG: 时序图，从上到下 4 个矩形图标代表 4 个角色（标 A B C D），纵向箭头依次表示请求和响应，每条箭头带方向和序号，布局为上下分层，白底，扁平，灰色和蓝色配色，无文字]]\`

❌ 反例 1（抽象）：\`[[IMG: 分库分表示意图]]\` ← AI 不知道画什么

❌ 反例 2（摄影风格）：\`[[IMG: 俯拍桌面，屏幕显示分库分表，干净背景]]\` ← 摄影风格，不是示意图

❌ 反例 3（试图让 AI 写字）：\`[[IMG: 架构图，含 Master 和 Shard 节点，标注了主从关系]]\` ← 文字会乱码

❌ 反例 4（装饰性描述）：\`[[IMG: 漂亮的分库分表架构示意图，简约现代风格]]\` ← 模糊词 + 装饰性描述
